    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/RETAR"
                ...
                Else
                    Call ShowConsoleMsg("Ingresa el nombre para aceptar el reto")
                End If

            Case "/ONLINE"    'COMANDO ONLINE
                Call WriteOnline
            ...
            'Case "/HOGAR" Se comenta la funcion de hogar, ya no es necesaria por las runas.
            '    Call WriteHome
                    ...
                    If ValidNumber(ArgumentosAll(0), eNumber_Types.ent_integer) Then
                        If CantidadArgumentos >= 2 Then 'puso el mapa?
                            If ValidNumber(ArgumentosAll(1), eNumber_Types.ent_integer) Then
                                Call WriteCuentaRegresiva(ArgumentosAll(0), ArgumentosAll(1))
                            Else
                                'Argumentos de mas?
                                Call ShowConsoleMsg("Valor incorrecto. Utilice /CR VALOR MAPA(Opcional).")
                            End If
                        Else
                            Call WriteCuentaRegresiva(ArgumentosAll(0), 0)
                        End If
                    Else
                        'No es valido el 1
                        Call ShowConsoleMsg("Valor incorrecto. Utilice /CR VALOR MAPA(Opcional).")
