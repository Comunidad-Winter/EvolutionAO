...
Sub MoveCharbyHead(ByVal CharIndex As Integer, ByVal nHeading As E_Heading)
    ...
    frmMain.imgChar.Top = charlist(CharIndex).Pos.Y - 2
    frmMain.imgChar.Left = charlist(CharIndex).Pos.X - 1
...
Sub RenderScreen(ByVal tilex As Integer, ByVal tiley As Integer, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer)
    ...
    'TODO : Check this!!
    If MapInfo.EsDungeon Then Exit Sub
...
Public Function RenderSounds()
    ...
    '**************************************************************
    If MapInfo.EsDungeon Then Exit Function
...
Sub ShowNextFrame(ByVal DisplayFormTop As Integer, ByVal DisplayFormLeft As Integer, ByVal MouseViewX As Integer, ByVal MouseViewY As Integer)
    ...
    If EngineRun Then
        ...
        If FadingTime > 0 Then
            FadingTime = FadingTime - 1
            'If ColourName > 0 Then
              '  ColourName = ColourName - 1
               ' Call RenderText(500, 300, MapInfo.Name, RGB(ColourName, ColourName, ColourName), frmConnect.font)
            'End If
            'Color del mapa al pasar (NOMBRE MAPA)
            Call RenderText(frmMain.MainViewShp.Width - frmConnect.TextWidth(MapInfo.Name) / 2 - 17, 270, MapInfo.Name, RGB(200, 200, 140), frmConnect.font)
        End If
        
        If DuracionPociones > 0 Then
            'Color al contador de dopa (CONTADOR CONTEO PARA QUE TERMINE LA DOPA)
            If DuracionPociones < 16 Then
                Call RenderText(710, 260, "DOPA: " & DuracionPociones & " segs", RGB(240, 255, 0), frmMain.font)
            End If
        End If
        
        If CountTime > 0 Then
            Call RenderText(frmMain.MainViewShp.Width - frmCargando.TextWidth(CStr(CountTime)) / 2 - 16, 300, CStr(CountTime), RGB(255, 255, 255), frmCargando.font)
        Else
            If CountFinish Then
                Call RenderText(frmMain.MainViewShp.Width - frmCargando.TextWidth("¡YA!") / 2 - 16, 300, "¡YA!", RGB(255, 37, 37), frmCargando.font)
            End If
        End If
        ...
        If FpsLimiter <> 0 Then
            'Limit FPS to 100 (an easy number higher than monitor's vertical refresh rates)
            While (DirectX.TickCount - fpsLastCheck) \ FpsLimiter < FramesPerSecCounter
                Sleep 5
            Wend
        End If
...
Private Sub CharRender(ByVal CharIndex As Long, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer)
    ...
    With charlist(CharIndex)
        ...
        'El nombre del clan y personal se muestra al navegar.
        If BodyIsBoat(.iBody) Then
                ...
                If .Head.Head(.Heading).GrhIndex Then
                    ...
                    If LenB(.Nombre) > 0 Then
                        ...
                        If Nombres Then
                            ...
                            'Cargamos el color del NPC guía
                            colorNpcGuia = RGB(0, 255, 219)
                            
                            'Seteamos color al NPC GUÍA
                            If (.iHead = 107 And .iBody = 257 And .Atacable = False) Then
                                Call RenderText(PixelOffsetX - ((frmMain.TextWidth(line) / 2) - 17), PixelOffsetY + 30, line, colorNpcGuia, frmMain.font)
                            End If
                            
                            'Seteamos color al NPC RAMÓN
                            If (.iHead = 47 And .iBody = 100 And .Atacable = False) Then
                                Call RenderText(PixelOffsetX - ((frmMain.TextWidth(line) / 2) - 17), PixelOffsetY + 30, line, colorNpcGuia, frmMain.font)
                            End If
                            
                            'Seteamos color al NPC FRODO
                            If (.iHead = 701 And .iBody = 389 And .Atacable = False) Then
                                Call RenderText(PixelOffsetX - ((frmMain.TextWidth(line) / 2) - 17), PixelOffsetY + 30, line, colorNpcGuia, frmMain.font)
                            End If
                            
                            'Seteamos color al NPC Mefisto
                            If (.iHead = 28 And .iBody = 243 And .Atacable = False) Then
                                Call RenderText(PixelOffsetX - ((frmMain.TextWidth(line) / 2) - 17), PixelOffsetY + 30, line, colorNpcGuia, frmMain.font)
                            End If
                            
                            'Seteamos color al NPC Grandalf
                            If (.iHead = 107 And .iBody = 490 And .Atacable = False) Then
                                Call RenderText(PixelOffsetX - ((frmMain.TextWidth(line) / 2) - 17), PixelOffsetY + 30, line, colorNpcGuia, frmMain.font)
                            End If
            ...
            Else
                ...
                If ClanTag(CharIndex) Then
                    ...
                    If .Body.Walk(.Heading).GrhIndex Then Call DDrawTransGrhtoSurface(.Body.Walk(.Heading), PixelOffsetX, PixelOffsetY, 1, 1)
                        ...
                        'Draw Helmet
                        If .Casco.Head(.Heading).GrhIndex Then Call DDrawTransGrhtoSurface(.Casco.Head(.Heading), PixelOffsetX + .Body.HeadOffset.X, PixelOffsetY + .Body.HeadOffset.Y + OFFSET_HEAD, 1, 0)
...
Public Sub Draw_Account_Grh(ByVal Slot As Byte, ByVal X As Byte, ByVal Y As Byte, ByVal file_Index As Long, ByRef N_Rect As RECT, ByRef AUX_Rect As RECT)
    With BackBufferSurface
        Call .BltFast(X, Y, SurfaceDB.Surface(file_Index), N_Rect, DDBLTFAST_SRCCOLORKEY Or DDBLTFAST_WAIT)
        Call .BltToDC(frmPanelAccount.picChar(Slot - 1).hdc, AUX_Rect, AUX_Rect)
    End With
End Sub

Public Sub Clear_Rect_Account(ByRef AUX_Rect As RECT)
    With BackBufferSurface
        Call .BltColorFill(AUX_Rect, 0)
    End With
End Sub
