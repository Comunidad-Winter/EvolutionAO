    ...
    If InMapBounds(map, X, Y) Then
        With UserList(UserIndex)
            If MapData(map, X, Y).NpcIndex > 0 Then     'Acciones NPCs
                ...
                ElseIf Npclist(tempIndex).NPCtype = eNPCType.Pasajes Then
                    ...
                    If Distancia(Npclist(tempIndex).Pos, .Pos) > 3 Then
                        Call WriteConsoleMsg(UserIndex, "Estás demasiado lejos del viajero.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If

                    Call WriteShowFormViajes(UserIndex, tempIndex)
