...
Sub ConnectNewUser(ByVal UserIndex As Integer, ByRef Name As String, ByVal UserRaza As eRaza, ByVal UserSexo As eGenero, ByVal UserClase As eClass, _
                   ByRef UserEmail As String, ByVal Hogar As eCiudad, ByVal Head As Integer, ByRef UserCaptcha As String)
    '*************************************************
    'Author: Unknown
    ...
    '[/Lherkiev] Remuevo los espacios aca
    Name = Trim$(Name)
    
    With UserList(UserIndex)
    
        If GetTickCount() And &H7FFFFFFF - UserList(UserIndex).LastTickCaptcha < 6000 Then
            Exit Sub
        End If
        ...
        If UCase$(UserCaptcha) <> UCase$(Chr$(.CaptchaCode(0)) & Chr$(.CaptchaCode(1)) & Chr$(.CaptchaCode(2)) & Chr$(.CaptchaCode(3))) Then
            Call WriteErrorMsg(UserIndex, "El captcha es invalido, intente nuevamente.")
        ...
        If Not EsGmChar(Name) Then
            ...
            .Stats.MaxSta = 20 * RandomNumber(1, .Stats.UserAtributos(eAtributos.Agilidad) \ 6)
...
#End If
        ...
        Call SaveUserToAccountCharfile(Name, .Account)
...
Sub CloseSocket(ByVal UserIndex As Integer)
    ...
    'Que pasa si te desconectas en pleno torneo....
    ...
    'Retos 1vs1,2vs2
    If UserList(UserIndex).mReto.Reto_Index <> 0 Then Call m_Retos1vs1.DisconnectUser_Reto(UserIndex)
    If UserList(UserIndex).sReto.Reto_Used Then Call m_Retos2vs2.Disconnect_Reto(UserIndex)

    Dim TmpInt As Integer
    TmpInt = NameIndex(UserList(UserIndex).mReto.Request_Name)

    ' @@ Mande reto.
    If TmpInt > 0 Then
        Call WriteConsoleMsg(TmpInt, UserList(UserIndex).Name & " se desconectó, se ha anulado la solicitud de reto.", FontTypeNames.FONTTYPE_INFO)
        Call Reset_UserReto(TmpInt)
...
Sub ConnectUser(ByVal UserIndex As Integer, ByRef Name As String)
    ...
    With UserList(UserIndex)
        ...
        If LenB(.PasswordLogged) < 1 Then Exit Sub
        If LenB(.Account) < 1 Then Exit Sub
        
        If Not CuentaExiste(.Account) Then
        ...
        If UCase$(.PasswordLogged) <> UCase$(LeerAccount.GetValue("INIT", "PASSWORD")) Then
            Set LeerAccount = Nothing
            Exit Sub
        End If
        
        Dim NumberOfCharacters As Byte
        NumberOfCharacters = val(LeerAccount.GetValue("INIT", "CANT_PJS"))

        Dim LoopC As Long

        For LoopC = 1 To NumberOfCharacters
            If UCase$(LeerAccount.GetValue("PJS", "PJ" & LoopC)) = UCase$(Name) Then
                Exit For
            End If
        Next LoopC
        ...
        If LoopC < 1 Then Exit Sub
        If LoopC > NumberOfCharacters Then Exit Sub
        ...
        If .Stats.ELV = 1 Then
            If .Pos.map = 195 Then
                If NumUsersLevelOne > 65 Then '65 es el valor total aceptado para el mapa 1
                    Call WriteErrorMsg(UserIndex, "Por favor espere para crear personaje dungeon newbie se encuentra lleno.")
                    Call FlushBuffer(UserIndex)
                    Call CloseSocket(UserIndex)
                    Exit Sub
                End If
        ...
        Call WriteChangeMap(UserIndex, .Pos.map, MapInfo(.Pos.map).Llueve)    'Carga el mapa
        Call WritePlayMidi(UserIndex, val(ReadField(1, MapInfo(.Pos.map).Music, 45)))

        'Si el usuario es Dios o Admin utiliza la siguiente tonalidad. (CHAT COLOR CHAT)
        If .flags.Privilegios = PlayerType.Admin Then
                .flags.ChatColor = RGB(232, 255, 0)
        ...
        ElseIf .flags.Privilegios <> PlayerType.User And .flags.Privilegios <> (PlayerType.User Or PlayerType.ChaosCouncil) And .flags.Privilegios <> (PlayerType.User Or PlayerType.RoyalCouncil) Then
            .flags.ChatColor = RGB(160, 255, 69)
...
#End If
        ...
        If Not EsGm(UserIndex) Then
            ...
            For loopX = 1 To LastUser
                Call WriteUpdateOnline(loopX)
            Next loopX
        End If
        ...
        'Mensajes automatizados desde servidor
        ...
        'For LoopR = 1 To UBound(.Mensajes())
                   'Call WriteConsoleMsg(UserIndex, "Anuncio automático> " & .Mensajes(LoopR), FontTypeNames.FONTTYPE_DIOS)
              'Next LoopR
        ...
        Call CheckRankingUser(UserIndex, TopFrags)
        Call CheckRankingUser(UserIndex, TopOro)
        Call CheckRankingUser(UserIndex, TopLevel)
        Call CheckRankingUser(UserIndex, TopRetos)
        Call CheckRankingUser(UserIndex, TopCriminales)
        Call CheckRankingUser(UserIndex, TopCiudadanos)
        
        If HappyHourActivated > 1 Then
            Call WriteConsoleMsg(UserIndex, "Atencion el Happy Hour esta en curso, aprovecha para entrenar con la exp aumentada.", FontTypeNames.FONTTYPE_INFOBOLD)
        End If
