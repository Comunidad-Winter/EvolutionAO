...
Public Sub DoLingotes(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        CantidadItems = MaximoInt(10, CInt((.Stats.ELV - 4) / 5))
...
Public Sub DoPescar(ByVal UserIndex As Integer)
    ...
    If res <= 6 Then
        ...
        Dim redPesca As Obj
        redPesca.ObjIndex = 543    'Red de pesca
        redPesca.Amount = 1

        If RandomNumber(1, 7000) = 7 And UserList(UserIndex).Stats.UserSkills(eSkill.Pesca) = 100 Then
            Call WriteConsoleMsg(UserIndex, "¡Red de Pesca!", FontTypeNames.FONTTYPE_INFO)
            Call MeterItemEnInventario(UserIndex, redPesca)
        End If
...
Public Function ObjEsRobable(ByVal VictimaIndex As Integer, ByVal Slot As Integer) As Boolean
    ...
    ObjData(OI).OBJType <> eOBJType.otLlaves And _
                   ...
                   ObjData(OI).OBJType <> eOBJType.otBarcos And ObjData(OI).Intransferible = 0
...
Public Sub DoApuñalar(ByVal UserIndex As Integer, ByVal VictimNpcIndex As Integer, ByVal VictimUserIndex As Integer, ByVal daño As Integer)
    ...
    If RandomNumber(0, 100) < Suerte Then
        If VictimUserIndex <> 0 Then
            ...
            Call WriteConsoleMsg(VictimUserIndex, "Te ha apuñalado " & UserList(UserIndex).Name & " por " & daño, FontTypeNames.FONTTYPE_GUILDMSG)

            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(UserList(VictimUserIndex).Char.CharIndex, FXAPUÑALAR, 0))
            
            If UserList(VictimUserIndex).flags.invisible < 1 Then
                Call SendData(SendTarget.ToIndex, UserIndex, PrepareMessageChatOverHead(daño, UserList(VictimUserIndex).Char.CharIndex, vbYellow))
            End If
        ...
        Else
            ...
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(Npclist(VictimNpcIndex).Char.CharIndex, FXAPUÑALAR, 0))
        End If
...
Public Sub DoGolpeCritico(ByVal UserIndex As Integer, ByVal VictimNpcIndex As Integer, ByVal VictimUserIndex As Integer, ByVal daño As Integer)
    ...
    If UserList(UserIndex).Invent.WeaponEqpSlot = 0 Then Exit Sub
    nameObj = ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Name
    If nameObj <> "Espada Vikinga" And nameObj <> "Espada critica" Then Exit Sub
    ...
    If RandomNumber(0, 100) < Suerte Then
        ...
        If VictimUserIndex <> 0 Then
            UserList(VictimUserIndex).Stats.MinHp = UserList(VictimUserIndex).Stats.MinHp - daño
            Call WriteConsoleMsg(UserIndex, "Has golpeado críticamente a " & UserList(VictimUserIndex).Name & " por " & daño & ".", FontTypeNames.FONTTYPE_DIOS)
            Call WriteConsoleMsg(VictimUserIndex, UserList(UserIndex).Name & " te ha golpeado críticamente por " & daño & ".", FontTypeNames.FONTTYPE_DIOS)
        Else
            Npclist(VictimNpcIndex).Stats.MinHp = Npclist(VictimNpcIndex).Stats.MinHp - daño
            Call WriteConsoleMsg(UserIndex, "Has golpeado críticamente a la criatura por " & daño & ".", FontTypeNames.FONTTYPE_DIOS)
            '[Alejo]
            Call CalcularDarExp(UserIndex, VictimNpcIndex, daño)
        End If
    End If
