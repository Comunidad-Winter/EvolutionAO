...
Public Function CrearNuevoClan(ByVal FundadorIndex As Integer, ByRef desc As String, ByRef GuildName As String, ByRef URL As String, ByRef codex() As String, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If Not TieneObjetos(COPA_OBJ_CLAN, COPA_AMOUNT, FundadorIndex) Then
        refError = "Para fundar un clan necesitas " & COPA_AMOUNT & " Pergaminos de Clan"
        Exit Function
    End If

    If Len(GuildName) < 3 Then
        refError = "Nombre de clan muy corto (Minimo 3 carácteres)."
        Exit Function
    End If

    If Len(GuildName) > 20 Then
        refError = "Nombre de clan muy largo. (Máximo 20 carácteres)"
        Exit Function
    End If
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 25 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 90 Then
        refError = "Para fundar un clan debes ser nivel 25, tener 10 pergaminos de clan y 90 skills en liderazgo."
        Exit Function
    End If

    ' If Not TieneObjetos(211, 1, UserIndex) Then
    ' refError = "Debes tener la gema de clan xd."
    ' Exit Function
    '  End If
    ...
    'call quitarobjetos(154,1,userindex)
