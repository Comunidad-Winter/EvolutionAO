...
Public ModificarExperiencia As Integer
Public ModificarOro As Integer
...
Public Function HayCura(ByVal UserIndex As Integer) As Boolean
...
Sub Main()
    ...
    ModificarExperiencia = val(GetVar(IniPath & "Server.ini", "INIT", "ModificarExperiencia"))
    ModificarOro = val(GetVar(IniPath & "Server.ini", "INIT", "ModificarOro"))
...
Public Sub EfectoVeneno(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Else
            ...
            If .sReto.Nick_Sender <> vbNullString Or .mReto.Request_Name <> vbNullString Then Exit Sub
...
Sub PasarSegundo()
    ...
    Call m_Retos1vs1.reto_all_loop
    Call m_Retos2vs2.Loop_reto

    For i = 1 To LastUser
    
        If UserList(i).flags.UserLogged Then

            Call m_Retos1vs1.Loop_UserReto(i)
            Call m_Retos2vs2.User_RetoLoop(i)

            If UserList(i).Counters.ViajeOBJ <> 0 Then
                UserList(i).Counters.ViajeOBJ = UserList(i).Counters.ViajeOBJ - 1
                If UserList(i).Counters.ViajeOBJ < 1 Then
                    Call WriteConsoleMsg(i, "Has llegado a tu destino.", FontTypeNames.FONTTYPE_INFO)
                    Call WarpUserCharX(i, UserList(i).PosViaje.map, UserList(i).PosViaje.X, UserList(i).PosViaje.Y, True)
                End If
            End If

            If UserList(i).Counters.Viaje <> 0 Then
                UserList(i).Counters.Viaje = UserList(i).Counters.Viaje - 1
                If UserList(i).Counters.Viaje < 1 Then
                    Call WriteConsoleMsg(i, "Has llegado a tu destino." & vbNewLine & "Fue un largo viaje, aliméntate y bebe algo para recuperarte", FontTypeNames.FONTTYPE_INFO)
                    Call WarpUserCharX(i, UserList(i).PosViaje.map, UserList(i).PosViaje.X, UserList(i).PosViaje.Y, True)
                    Call UpdateStatsViaje(i)
                End If
            End If
            ...
            If UserList(i).Counters.Saliendo Then
                ...
                If UserList(i).Counters.Salir < 1 Then
                    Call WriteConsoleMsg(i, "Gracias por jugar Evolution Online", FontTypeNames.FONTTYPE_INFO)
...
Public Sub CargarELU()
    ...
    Call Leer.Initialize(DatPath & "Niveles.dat")

    Dim LoopC As Long

    For LoopC = 1 To STAT_MAXELV
        LevelELU(LoopC) = CLng(Leer.GetValue("INIT", "Nivel" & LoopC))
    Next LoopC
