...
On Error GoTo errorh:
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Se canceló por falta de participantes.", FontTypeNames.FONTTYPE_SERVER))
    Dim i As Integer
...
On Error GoTo errorh
    ...
    Torneo_Activo = False
    Torneo_Esperando = False
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Cancelado por un Game Master.", FontTypeNames.FONTTYPE_SERVER))
    Dim i As Integer
    For i = LBound(Torneo_Luchadores) To UBound(Torneo_Luchadores)
                If (Torneo_Luchadores(i) <> -1) Then
                        Dim NuevaPos As WorldPos
...
Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UserIndex).Name & " se fue del torneo mientras esperaba pelear!", FontTypeNames.FONTTYPE_SERVER))
...
End If
        ...
        If (Real) Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UserIndex).Name & " pierde el combate!", FontTypeNames.FONTTYPE_SERVER))
        Else
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UserIndex).Name & " se fue del combate!", FontTypeNames.FONTTYPE_SERVER))
        End If
    ...
    If (Torneo_Rondas = 1) Then
        ...
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UserIndex).Name & " pierde el combate!", FontTypeNames.FONTTYPE_SERVER))
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Ganador del torneo: " & UserList(Torneo_Luchadores(LI1)).Name, FontTypeNames.FONTTYPE_SERVER))
    ...
    End If
        ...
        If (2 ^ Torneo_Rondas = 2 * combate) Then
                ...
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Siguiente ronda!", FontTypeNames.FONTTYPE_SERVER))
...
On Error GoTo errorh
Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UserIndex).Name & " ha desconectado en torneo automático, se le penaliza con 500K!!", FontTypeNames.FONTTYPE_SERVER))
...
If (Torneo_Activo) Then
        ...
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Esta empezando un nuevo torneo 1VS1 de " & val(2 ^ rondas) & " participantes!! para participar teclea /PARTICIPAR - (No cae inventario)", FontTypeNames.FONTTYPE_SERVER))
...
On Error GoTo errorh
        ...
        If (Torneo_Luchadores(i) = -1) Then
               ...
               Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> El personaje " & UserList(UserIndex).Name & " ingresó al torneo.", FontTypeNames.FONTTYPE_SERVER))
                If (i = UBound(Torneo_Luchadores)) Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Se da inicio al torneo.", FontTypeNames.FONTTYPE_SERVER))
                Torneo_Esperando = False
                Call Rondas_Combate(1)
...
Dim UI1 As Integer, UI2 As Integer
    ...
    If (UI1 = -1) Then
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Combate anulado por la desconexión de uno de los dos participantes.", FontTypeNames.FONTTYPE_SERVER))
        If (Torneo_Rondas = 1) Then
            If (UI2 <> -1) Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> Torneo terminado, ganador del torneo por eliminación " & UserList(UI2).Name & ".", FontTypeNames.FONTTYPE_SERVER))
                UserList(UI2).flags.automatico = False
                ' dale_recompensa()
                Torneo_Activo = False
                Exit Sub
            End If
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> No hay ganador del evento por la desconexión de todos sus participantes.", FontTypeNames.FONTTYPE_SERVER))
            Exit Sub
        End If
        If (UI2 <> -1) Then _
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> El usuario " & UserList(UI2).Name & " pasó a la siguiente ronda.", FontTypeNames.FONTTYPE_SERVER))
        ...
        Exit Sub
    End If
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Torneo Automático> " & UserList(UI1).Name & " vs. " & UserList(UI2).Name & ". Esquinas!!, Peleen!", FontTypeNames.FONTTYPE_SERVER))
