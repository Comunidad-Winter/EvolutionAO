...
Function PuedeLanzar(ByVal UserIndex As Integer, ByVal HechizoIndex As Integer) As Boolean
    ...
    With UserList(UserIndex)
        ...
        If .Stats.UserSkills(eSkill.Magia) < Hechizos(HechizoIndex).MinSkill Then
            Call WriteConsoleMsg(UserIndex, "Debes tener " & Hechizos(HechizoIndex).MinSkill & " skills en mágia.", FontTypeNames.FONTTYPE_INFO)
            Exit Function
        End If

        If .Stats.MinSta < Hechizos(HechizoIndex).StaRequerido Then
            If .Genero = eGenero.Hombre Then
                Call WriteConsoleMsg(UserIndex, "Estás muy cansado para lanzar este hechizo, verifica que tengas las barras de abajo a la derecha llenas de HAMBRE y SED.", FontTypeNames.FONTTYPE_INFO)
            Else
                Call WriteConsoleMsg(UserIndex, "Estás muy cansada para lanzar este hechizo, verifica que tengas las barras de abajo a la derecha llenas de HAMBRE y SED.", FontTypeNames.FONTTYPE_INFO)
            End If
            Exit Function
        End If
        ...
        If .Stats.MinMAN < Hechizos(HechizoIndex).ManaRequerido * DruidManaBonus Then
            Call WriteConsoleMsg(UserIndex, "No tienes suficiente maná, apriete la tecla F6 para meditar o bien compra pociones azules y utilízalas.", FontTypeNames.FONTTYPE_INFO)
            Exit Function
        End If
...
Sub HechizoInvocacion(ByVal UserIndex As Integer, ByRef HechizoCasteado As Boolean)
    ...
    With UserList(UserIndex)
        ...
        If .mReto.Reto_Index > 0 Then
            Call WriteConsoleMsg(UserIndex, "No puedes invocar criaturas en retos 1vs1.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        If .sReto.Reto_Index > 0 Then
            Call WriteConsoleMsg(UserIndex, "No puedes invocar criaturas en retos 2vs2.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
...
Sub HechizoEstadoUsuario(ByVal UserIndex As Integer, ByRef HechizoCasteado As Boolean)
    ...
    With UserList(UserIndex)
        ...
        If Hechizos(HechizoIndex).Invisibilidad = 1 Then
            ...
            Call WriteRemoveCharDialog(TargetIndex, UserList(TargetIndex).Char.CharIndex)
        ...
        If Hechizos(HechizoIndex).Revivir = 1 Then
            If UserList(TargetIndex).flags.Muerto = 1 Then
                ...
                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(UserList(TargetIndex).Char.CharIndex, Hechizos(HechizoIndex).FXgrh, 0))
            Else
                HechizoCasteado = False
            End If
...
Public Function HechizoPropUsuario(ByVal UserIndex As Integer) As Boolean
    ...
    With UserList(TargetIndex)
        ...
        ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
            ...
            Call WriteChatOverHead(UserIndex, daño, .Char.CharIndex, vbRed)
