...
Sub CargarMsgs()
        ...
        rPath = App.Path & "\Dat\Anuncios.ini"
...
Public Sub GrabarMapa(ByVal map As Long, ByRef MAPFILE As String)
    ...
    For Y = YMinMapSize To YMaxMapSize
        For X = XMinMapSize To XMaxMapSize
            With MapData(map, X, Y)
                ...
                If .NpcIndex Then
                    NpcInvalido = (Npclist(.NpcIndex).NPCtype = eNPCType.Pretoriano) Or (Npclist(.NpcIndex).MaestroUser > 0) Or (Npclist(.NpcIndex).Numero = 16)
    ...
    With MapInfo(map)
        ...
        Call IniManager.ChangeValue("Mapa" & map, "Pk", IIf(.Pk, "0", "1"))
        ...
        Call IniManager.ChangeValue("Mapa" & map, "AtaqueFaccionario", .AtaqueFaccionario)
    ...
    For Object = 1 To NumObjDatas
        With ObjData(Object)
            ...
            Select Case .OBJType
                ...
                Case eOBJType.otObjetoViajero

                    Dim Coord As String
                    Coord = CStr(Leer.GetValue("OBJ" & Object, "ObjViajePos"))

                    .PasajePos.map = CInt(ReadField(1, Coord, 45))
                    .PasajePos.X = CInt(ReadField(2, Coord, 45))
                    .PasajePos.Y = CInt(ReadField(3, Coord, 45))
...
Errhandler:
    ...
    Call LogError("Error cargando el objeto n: " & Object & " verificar el dateo.")
...
Sub LoadUserStats(ByVal UserIndex As Integer, ByRef UserFile As clsIniReader)
    ...
    With UserList(UserIndex)
        With .Stats
            ...
            .PuntosCanje = CInt(UserFile.GetValue("STATS", "PUNTOSCANJE"))
            ...
            .RetosGanados = CInt(UserFile.GetValue("MUERTES", "RetosGanados"))
            .RetosPerdidos = CInt(UserFile.GetValue("MUERTES", "RetosPerdidos"))
        End With
...
#End If
        ...
        For LoopC = 1 To MAX_INVENTORY_SLOTS
            ...
            If ln = vbNullString Then ln = 0 - 0 - 0
...
Sub LoadMapData()
    ...
    tFileName = App.Path & MapPath & "Mapa"

    For map = 1 To NumMaps
        Call CargarMapa(map, tFileName & map)

        If MapInfo(map).BackUp <> 0 Then
            CantMaps = CantMaps + 1
            ReDim Preserve MapBackup(1 To CantMaps) As Integer
            MapBackup(CantMaps) = map
        End If
...
Public Sub CargarMapa(ByVal map As Long, ByVal MAPFl As String)
    ...
    If Not FileExist(MAPFl & ".map", vbArchive) Then
        ...
        For X = XMinMapSize To XMaxMapSize
            With MapData(map, X, Y)
                ...
                If ByFlags And 2 Then
                    ...
                    For LoopC = 1 To MAX_COSTAS
                        If .Graphic(2) = Array_Costas_Agua(LoopC) Then
                            .Graphic(2) = 0
                            Exit For
                        End If
                    Next LoopC
                ...
                If ByFlags And 2 Then
                    ...
                    If .NpcIndex > 0 Then
                        ...
                        If Npclist(.NpcIndex).Pos.map <> 0 And Npclist(.NpcIndex).Pos.X <> 0 And Npclist(.NpcIndex).Pos.Y <> 0 Then
    ...
    With MapInfo(map)
        ...
        .LevelMin = val(Leer.GetValue("Mapa" & map, "ElvMin"))
        .LevelMax = val(Leer.GetValue("Mapa" & map, "ElvMax"))

        If .LevelMax < 0 Then
            .LevelMax = STAT_MAXELV
        End If
        ...
        .Llueve = IIf(.Zona = "DUNGEON", 1, 0)

        .Restringir = Leer.GetValue("Mapa" & map, "Restringir")
        .BackUp = val(Leer.GetValue("Mapa" & map, "Backup"))
        .AtaqueFaccionario = val(Leer.GetValue("Mapa" & map, "AtaqueFaccionario"))
    End With
...
#End If
        ...
        Call Manager.ChangeValue("STATS", "PUNTOSCANJE", CStr(.Stats.PuntosCanje))
        ...
        Call Manager.ChangeValue("MUERTES", "RetosGanados", CStr(.Stats.RetosGanados))
        Call Manager.ChangeValue("MUERTES", "RetosPerdidos", CStr(.Stats.RetosPerdidos))
...
Public Function getLimit(ByVal Mapa As Integer, ByVal side As Byte) As Integer
    ...
    If Mapa < 1 Then Exit Function
    If Mapa > NumMaps Then Exit Function
