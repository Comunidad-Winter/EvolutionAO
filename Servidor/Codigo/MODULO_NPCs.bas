...
Public Sub MakeNPCChar(ByVal toMap As Boolean, sndIndex As Integer, NpcIndex As Integer, ByVal map As Integer, ByVal X As Integer, ByVal Y As Integer)
    ...
    'Cargamos el boolean del NPC si lo muestra...
    showName = val(GetVar(DatPath & "NPCs.dat", "NPC" & Npclist(NpcIndex).Numero, "ShowName"))
    nombreCargado = IIf(showName > 0, Npclist(NpcIndex).Name, vbNullString)
    
    'Cargamos valores del NPC
    MapData(map, X, Y).NpcIndex = NpcIndex
    
    If Not toMap Then
        Call WriteCharacterCreate(sndIndex, Npclist(NpcIndex).Char.Body, Npclist(NpcIndex).Char.Head, Npclist(NpcIndex).Char.heading, Npclist(NpcIndex).Char.CharIndex, X, Y, 0, 0, 0, 0, 0, nombreCargado, 0, 0)
...
Public Function OpenNPC(ByVal NpcNumber As Integer, Optional ByVal Respawn = True) As Integer
    ...
    With Npclist(NpcIndex)
        ...
        If .NPCtype = eNPCType.Pasajes Then

            StrCiudades() = Split(Leer.GetValue("NPC" & NpcNumber, "Ciudades"), "-")
            MAX_CIUDADES = UBound(StrCiudades())

            If MAX_CIUDADES <> -1 Then
                ReDim .Ciudades(0 To MAX_CIUDADES) As Byte

                For LoopC = 0 To MAX_CIUDADES
                    .Ciudades(LoopC) = val(StrCiudades(LoopC))
                Next LoopC
            End If
        ...
        .GiveEXP = val(Leer.GetValue("NPC" & NpcNumber, "GiveEXP")) * ModificarExperiencia
