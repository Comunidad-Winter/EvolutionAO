...
Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    With Npclist(NpcIndex)
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, PrepareMessageChatOverHead(daño, .Char.CharIndex, vbRed))
        ...
        If .Stats.MinHp > 0 Then
            ...
            Else
                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(Npclist(NpcIndex).Char.CharIndex, FXGOLPE, 0))
            End If
...
Public Sub NpcDaño(ByVal NpcIndex As Integer, ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Call SendData(SendTarget.ToIndex, NpcIndex, PrepareMessageChatOverHead(daño, Npclist(NpcIndex).Char.CharIndex, vbRed))
...
Public Function NpcAtacaUser(ByVal NpcIndex As Integer, ByVal UserIndex As Integer) As Boolean
    ...
    Else
        ...
        'Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead("Esquivas el ataque de " & Npclist(NpcIndex).name, UserList(UserIndex).Char.CharIndex, vbYellow))
...
Public Sub UserDañoUser(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    With UserList(AtacanteIndex)
        ...
        If UserList(VictimaIndex).flags.invisible < 1 Then
            Call SendData(SendTarget.ToIndex, AtacanteIndex, PrepareMessageChatOverHead(daño, UserList(VictimaIndex).Char.CharIndex, vbRed))
        End If
        ...
        If .flags.Hambre = 0 And .flags.Sed = 0 Then
            ...
            Else
                Call SendData(SendTarget.ToPCArea, AtacanteIndex, PrepareMessageCreateFX(UserList(VictimaIndex).Char.CharIndex, FXGOLPE, 0))
            End If
...
Public Function PuedeAtacar(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer) As Boolean
    ...
    If SameParty(AttackerIndex, VictimIndex) Then
        Call WriteConsoleMsg(AttackerIndex, "No puedes atacar usuarios de tu misma party.", FontTypeNames.FONTTYPE_INFO)
        Exit Function
    End If

    ' @@ No puede atacar antes del conteo.
    ...
    ' @@ No puede atacar a su compañero en retos 2vs2
    If UserList(AttackerIndex).sReto.Reto_Used Then
        If m_Retos2vs2.Can_Attack(AttackerIndex, VictimIndex) = False Then
            Call WriteConsoleMsg(AttackerIndex, "No puedes atacar a tu compañero.", FontTypeNames.FONTTYPE_FIGHT)
            PuedeAtacar = False
            Exit Function
        End If
    End If
    
    If MapInfo(UserList(AttackerIndex).Pos.map).AtaqueFaccionario > 0 Then
        If UserList(AttackerIndex).Faccion.ArmadaReal < 1 And UserList(AttackerIndex).Faccion.FuerzasCaos < 1 Then
            Call WriteConsoleMsg(AttackerIndex, "En este mapa solo pueden atacarse usuarios faccionarios.", FontTypeNames.FONTTYPE_FIGHT)
            PuedeAtacar = False
            Exit Function
        End If
    End If
