            ...
            If .TileExit.map > 0 And .TileExit.map <= NumMaps Then
                ...
                ElseIf UCase$(MapInfo(DestPos.map).Restringir) = "FACCION" Then    '¿Es mapa de faccionarios?
                    ...
                    Else    'No es Faccionario
                        ...
                        'Puede q salte error
                    End If
                ElseIf MapInfo(DestPos.map).LevelMax < UserList(UserIndex).Stats.ELV Or MapInfo(DestPos.map).LevelMin > UserList(UserIndex).Stats.ELV Then
                    Call WriteConsoleMsg(UserIndex, "Tú nivel tiene que ser mayor a " & MapInfo(DestPos.map).LevelMin & " y menor que " & MapInfo(DestPos.map).LevelMax + 1 & " para entrar a este mapa.", FontTypeNames.FONTTYPE_INFO)
                    Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
...
Sub ClosestLegalPos(Pos As WorldPos, ByRef nPos As WorldPos, Optional PuedeAgua As Boolean = False, Optional PuedeTierra As Boolean = True)
    ...
    If Notfound Or MapData(nPos.map, nPos.X, nPos.Y).TileExit.map > 0 Then
        nPos.X = 0
        nPos.Y = 0
    End If
    ...
    For LoopC = 1 To LastUser
        If UserList(LoopC).flags.UserLogged Then
            If UserList(LoopC).ip = UserIP And UserIndex <> LoopC Then
                ...
                If CountTrab > 0 Then
                    If UserList(LoopC).clase = eClass.Worker Then
                        CheckForSameIP = True
                        Exit Function
                    End If
                End If
                        ...
                        If LenB(.DescRM) = 0 And .showName Then    'No tiene descRM y quiere que se vea su nombre.
                            ...
                            Else
                                If Not .flags.Privilegios And PlayerType.User Then
                                    ...
                                    'Si es Administrador. (16)
                                    If .flags.Privilegios = 16 Then
                                        Stat = Stat & " <Administrador>"
                                        
                                    'Si es Dios (8)
                                    ElseIf .flags.Privilegios = 16 Then
                                        Stat = Stat & " <Dios>"
                                        
                                    'Si es Semi Dios (4)
                                    ElseIf .flags.Privilegios = 4 Then
                                        Stat = Stat & " <Semi Dios>"
                                    
                                    'Si es Consejero (2)
                                    ElseIf .flags.Privilegios = 2 Then
                                    Stat = Stat & " <Consejero>"
                                    
                                    'Si es RolerMaster (34)
                                    ElseIf .flags.Privilegios = 34 Then
                                        Stat = Stat & " <Roler Master>"
                                    End If
                                ...
                                ElseIf criminal(TempCharIndex) Then
                                    Stat = Stat & " <CRIMINAL> ~255~37~37~1~0"
                                Else
                                    Stat = Stat & " <CIUDADANO> ~0~187~255~1~0"
                                End If
                            End If
                            
                            If .Stats.RetosGanados > 0 Then
                                Stat = Stat & " (Retos: " & .Stats.RetosGanados & ")"
                            End If
