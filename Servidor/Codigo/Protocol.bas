...
Private Enum ServerPacketID
    ...
    UpdateOnline            'Actualizar onlines
    SeeInProcess
    ShowFormViajes          'Sistema de viajes
    AccountLogged
...
Private Enum ClientPacketID
    ...
    SCAPTIONS
    Ping                    '/PING}
    Torneo                  '/NUEVOT  (numero)
    'Para que funcione, usamos el timer, o si lo querés hacer vía un GM, ponés /NUEVOT (Y UN NUMERO DEL 1 AL 5)
    'Los valores son así.
    '/NUEVOT 1 = 2 participantes.
    '/NUEVOT 2 = 4 participantes.
    '/NUEVOT 3 = 8 participantes.
    '/NUEVOT 4 = 16 participantes.
    '/NUEVOT 5 = 32 participantes.
    'Si el gm hizo el evento, y los cupos no fueron llenados, el mismo debe teclear /CANCELART y llevará automáticamente todos los usuarios a ullathorpe.
    'El timer, posee que si no se llena a los 2 minutos, lleva a todos a ulla...
    RequestPartyForm
    ItemUpgrade
    GMCommands
    InitCrafting
...
'Declaramos variables de torneo
...
Public Enum FontTypeNames
    ...
    FONTTYPE_CONSE
    FONTTYPE_DIOS
    FONTTYPE_ADMIN 'Agregamos color Admin
End Enum
...
Public Function HandleIncomingData(ByVal UserIndex As Integer) As Boolean
    ...
    If Not (PacketID = ClientPacketID.LoginExistingChar _
            ...
            Or PacketID = ClientPacketID.RecoverAccount _
            Or PacketID = ClientPacketID.ChangePasswordAccount _
            Or PacketID = ClientPacketID.DeleteCharAccount) Then
    ...
    Select Case PacketID
        ...
        Case ClientPacketID.Reset                   '/RESET
            Call HandleReset(UserIndex)
        ...
        Select Case UserList(UserIndex).incomingData.PeekByte() '/Verificamos /NUEVOT (TORNEOS)
        
            Case TorneoPacketID.ATorneo 'INICIA
                Call HandleArrancaTorneo(UserIndex)
    
            Case TorneoPacketID.CTorneo 'CANCELAR
                Call HandleCancelaTorneo(UserIndex)
                
            Case TorneoPacketID.PTorneo 'PARTICIPAR
        ...
        Case ClientPacketID.RecoverAccount
            Call HandleRecoverAccount(UserIndex)

        Case ClientPacketID.ChangePasswordAccount
            Call HandleChangePasswordAccount(UserIndex)

        Case ClientPacketID.DeleteCharAccount
            Call HandleDeleteCharAccount(UserIndex)
...
Private Sub HandleGMCommands(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Select Case Command
            ...
            Case eGMCommands.CuentaRegresiva               '/CUENTAREGRESIVA NUMERO MAPA
...
Private Sub HandleLoginExistingChar(ByVal UserIndex As Integer)
    ...
    If Not BANCheck(UserName) Then
        Call ConnectUser(UserIndex, UserName)
    Else
        Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Evolution Online debido a tu mal comportamiento. Puedes consultar el reglamento y el sistema de soporte desde www.argentumonline.com.ar")
    End If
...
Private Sub HandleLoginNewChar(ByVal UserIndex As Integer)
    ...
    UserCaptcha = Buffer.ReadASCIIString()

    Call ConnectNewUser(UserIndex, UserName, Race, Gender, Class, UserName, Homeland, Head, UserCaptcha)
...
Private Sub HandleWalk(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If .Counters.ViajeOBJ <> 0 Then
            Call WriteConsoleMsg(UserIndex, "Tu viaje ha sido cancelado, ten cuidado de no moverte al concentrarte para viajar", FontTypeNames.FONTTYPE_INFO)
            .Counters.ViajeOBJ = 0
        End If
        ...
        Select Case Skill
            ...
            Case Ocultarse
                ...
                If .mReto.Reto_Index > 0 Then
                    Call WriteConsoleMsg(UserIndex, "No puedes ocultarte si estás en retos 1vs1.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
                
                If .sReto.Reto_Index > 0 Then
                    Call WriteConsoleMsg(UserIndex, "No puedes ocultarte si estás en retos 2vs2.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
Private Sub HandleCreateNewGuild(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If modGuilds.CrearNuevoClan(UserIndex, desc, GuildName, site, codex, .FundandoGuildAlineacion, ErrorStr) Then
            ...
            Call QuitarObjetos(COPA_OBJ_CLAN, COPA_AMOUNT, UserIndex)
...
Private Sub HandleOnline(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Next i
        Call WriteConsoleMsg(UserIndex, "Record de usuarios conectados: " & CStr(recordusuarios), FontTypeNames.FONTTYPE_INFOBOLD)
        Call WriteConsoleMsg(UserIndex, "Usuarios conectados actualmente: " & CStr(Count), FontTypeNames.FONTTYPE_INFOBOLD)
'        Call WriteConsoleMsg(UserIndex, LCase$(str), FontTypeNames.FONTTYPE_INFOBOLD)
...
Public Sub HandleReset(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If .Stats.ELV > 15 Or .flags.Muerto = 1 Then  ' SI ES MAYOR O IGUAL A X NIVEL No podes resetear
            Call WriteConsoleMsg(UserIndex, "Para resetear el personaje necesitas ser nivel 15 o inferior.. ¡y no puedes estar muerto!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        If .Pos.map = 195 Then
            NumUsersLevelOne = NumUsersLevelOne - 1
            If NumUsersLevelOne < 0 Then NumUsersLevelOne = 0
        End If
        
        .flags.Muerto = 0
        .flags.Escondido = 0
        
        .UltimoGlobal = GetTickCount() And &H7FFFFFFF
        ...
        If .clase <> eClass.Worker Then
            .Stats.SkillPts = 10 '++ Si no es trabajador
        Else
            .Stats.SkillPts = 500    '++ Si es trabajador
        End If
        ...
        ElseIf .clase = eClass.Cleric Or .clase = eClass.Druid _
            ...
            .Stats.MaxMAN = 50
            .Stats.MinMAN = 50
        ElseIf .clase = eClass.Bandit Then    'Mana Inicial del Bandido (ToxicWaste)
            .Stats.MaxMAN = 50
            .Stats.MinMAN = 50
        Else
            .Stats.MaxMAN = 0
        ...
        If .clase = eClass.Mage Or .clase = eClass.Cleric Or _
           .clase = eClass.Druid Or .clase = eClass.Bard Or _
           .clase = eClass.Assasin Then
            .Stats.UserHechizos(1) = 2

            If .clase = eClass.Druid Then .Stats.UserHechizos(2) = 46
        End If
...
Private Sub HandleMeditate(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If .flags.Meditando Then
            ...
            If .Faccion.ArmadaReal Then
                .Char.FX = FXIDs.FXMEDITARARMADA
            ElseIf .Faccion.FuerzasCaos Then
                .Char.FX = FXIDs.FXMEDITARLEGION
...
Private Sub HandleEnlist(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        'aca analiza si soy para pk o real
...
Private Sub HandleGMRequest(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If Not Ayuda.Existe(.Name) Then
            ...
            Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(LCase$(.Name) & "> Ha solicitado ayuda mediante el comando /GM , tipea /SHOW SOS para socorrerlo.", FontTypeNames.FONTTYPE_GUILDMSG))
...
Private Sub HandleDenounce(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If Not IntervaloPermiteDenunciar(UserIndex) Then
            Call WriteConsoleMsg(UserIndex, "Debes esperar 20 segundos para volver a denunciar.", FontTypeNames.FONTTYPE_DIOS)
            Exit Sub
        End If
...
Private Sub HandleGuildFundate(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If TieneObjetos("406", 10, UserIndex) Then
            Call WriteShowGuildAlign(UserIndex)
        Else
            Call WriteConsoleMsg(UserIndex, "Necesitas 10 Pergaminos de Clan para fundar.", FontTypeNames.FONTTYPE_INFOBOLD)
        End If
    End With
End Sub
                ...
                If tUser < 1 Then
                    If InMapBounds(map, X, Y) Then
                        ...
                        Else
                            ...
                            Call WriteConsoleMsg(UserIndex, "El personaje se encuentra offline , se ha transportado de todas formas a las cordenadas indicadas.", FontTypeNames.FONTTYPE_INFO)
                            Call LogGM(.Name, "Transportó a " & UserName & " hacia " & "Mapa" & map & " X:" & X & " Y:" & Y)
                        End If
                    Else
                        Call WriteConsoleMsg(UserIndex, "Posición invalida.", FontTypeNames.FONTTYPE_INFO)
                    End If
...
Private Sub HandleRequestUserList(ByVal UserIndex As Integer)
    ...
    If Count > 1 Then
        Call WriteUserNameList(UserIndex, names(), Count - 1)
    End If
        ...
        If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (Not .flags.Privilegios And PlayerType.User) <> 0 Then
            ...
            Else
                ...
                ElseIf tUser <= 0 Then
                    ...
                    Call WriteVar(CharPath & UserName & ".chr", "PENAS", "P" & Count + 1, LCase$(.Name) & ": CARCEL " & jailTime & "m, Razón: " & LCase$(Reason) & " " & Date & " " & time)
                    ...
                    Call WriteConsoleMsg(UserIndex, "El personaje se encuentra OFFLINE , de todas formas has encarcelado al personaje " & UserName & " por " & jailTime & " minutos .", FontTypeNames.FONTTYPE_INFO)
                    Rem Call WriteConsoleMsg(UserIndex, "El personaje se encuentra OFFLINE , de todas formas ha sido encarcelado por.", FontTypeNames.FONTTYPE_INFO)
                ...
                Else
                    ...
                    Else
                        ...
                        Call WriteConsoleMsg(UserIndex, "Has encarcelado al personaje " & UserName & " por " & jailTime & " minutos.", FontTypeNames.FONTTYPE_INFO)
                    End If
                End If
            End If
        End If
        ...
        If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) <> 0 Then
            ...
            If tUser > 0 Then
                ...
                Else
                    ...
                    If UserList(tUser).mReto.Reto_Index > 0 Then
                        Call WriteConsoleMsg(UserIndex, "No puedes ejecutarlo esta en reto 1vs1.", FontTypeNames.FONTTYPE_INFO)
                        Call .incomingData.CopyBuffer(Buffer)
                        Exit Sub
                    End If
                    
                    If UserList(tUser).sReto.Reto_Index > 0 Then
                        Call WriteConsoleMsg(UserIndex, "No puedes ejecutarlo esta en reto 2vs2.", FontTypeNames.FONTTYPE_INFO)
                        Call .incomingData.CopyBuffer(Buffer)
                        Exit Sub
                    End If
        ...
        If (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) Then
            ...
            Else
                If (.flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin)) <> 0 Or (UserList(tUser).flags.Privilegios And (PlayerType.Consejero Or PlayerType.User)) <> 0 Then
                    ...
                    If UserList(tUser).mReto.Reto_Index > 0 Then
                        Call WriteConsoleMsg(UserIndex, "No puedes sumonearlo esta en reto 1vs1.", FontTypeNames.FONTTYPE_INFO)
                        Call .incomingData.CopyBuffer(Buffer)
                        Exit Sub
                    End If
                    
                    If UserList(tUser).sReto.Reto_Index > 0 Then
                        Call WriteConsoleMsg(UserIndex, "No puedes sumonearlo esta en reto 2vs2.", FontTypeNames.FONTTYPE_INFO)
                        Call .incomingData.CopyBuffer(Buffer)
                        Exit Sub
                    End If
            ...
            For X = .Pos.X - MinXBorder + 1 To .Pos.X + MinXBorder - 1
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                    If MapData(.Pos.map, X, Y).ObjInfo.ObjIndex > 0 Then
                        If MapData(.Pos.map, X, Y).Blocked <> 1 Then
                ...
                If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                    ...
                    If Count > 0 Then
                        ...
                        For LoopC = 1 To Count
                            Call WriteVar(CharPath & UserName & ".chr", "PENAS", "P" & Count, "")
                        Next LoopC
                        
                        Call WriteVar(CharPath & UserName & ".chr", "PENAS", "Cant", "0")
                    End If
                    
                    Call WriteConsoleMsg(UserIndex, "Penas borradas.", FontTypeNames.FONTTYPE_INFO)
                    
                    Dim N As Integer
                    N = NameIndex(UserName)
                    
                    If N > 0 Then
                        If UserList(N).Counters.Pena > 0 Then
                            UserList(N).Counters.Pena = 0
                            Call WarpUserChar(N, Libertad.map, Libertad.X, Libertad.Y, True)
                            Call WriteConsoleMsg(N, "¡Has sido liberado!", FontTypeNames.FONTTYPE_INFO)
                        End If
                    End If
...
Public Sub HandleChatColor(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If (.flags.Privilegios And (PlayerType.Dios Or PlayerType.RoleMaster)) Then
...
Public Sub WriteLoggedMessage(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex).outgoingData
        ...
        Call .WriteByte(ServerPacketID.Logged)

        '++ Intervalos
        Call .WriteInteger(IntervaloUserPuedeCastear)
        Call .WriteInteger(IntervaloUserPuedeAtacar)
        Call .WriteInteger(IntervaloMagiaGolpe)

        Call .WriteInteger(IntervaloUserPuedeTrabajar)
        Call .WriteInteger(IntervaloUserPuedeUsar)
        Call .WriteInteger(IntervaloUserPuedeUsarClick)
        Call .WriteInteger(IntervaloFlechasCazadores)
...
Public Sub WriteUpdateMana(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateMana)
        Call .outgoingData.WriteInteger(.Stats.MinMAN)

        If .ElPedidorSeguimiento > 0 Then
            Call WriteUpdateSegMANA(UserIndex)
        End If
    End With
...
Public Sub WriteUpdateHP(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateHP)
        Call .outgoingData.WriteInteger(.Stats.MinHp)

        If .ElPedidorSeguimiento > 0 Then
            Call WriteUpdateSegHP(UserIndex)
        End If
    End With
...
Public Sub WriteChangeMap(ByVal UserIndex As Integer, ByVal map As Integer, ByVal Llueve As Byte)
    '***************************************************
    'Author: Juan Martín Sotuyo Dodero (Maraxus)
    'Last Modification: 05/17/06
    'Writes the "ChangeMap" message to the given user's outgoing data buffer
...
Public Sub WriteUpdateUserStats(ByVal UserIndex As Integer)
    ...
    On Error GoTo Errhandler
    With UserList(UserIndex)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateUserStats)
        Call .outgoingData.WriteInteger(.Stats.MaxHp)
        Call .outgoingData.WriteInteger(.Stats.MinHp)
        Call .outgoingData.WriteInteger(.Stats.MaxMAN)
        Call .outgoingData.WriteInteger(.Stats.MinMAN)
        Call .outgoingData.WriteInteger(.Stats.MaxSta)
        Call .outgoingData.WriteInteger(.Stats.MinSta)
        Call .outgoingData.WriteLong(.Stats.GLD)
        Call .outgoingData.WriteByte(.Stats.ELV)
        Call .outgoingData.WriteLong(.Stats.ELU)
        Call .outgoingData.WriteLong(.Stats.Exp)
...
Public Sub WriteUpdateOnline(ByVal UserIndex As Integer)

    On Error GoTo Errhandler

    With UserList(UserIndex).outgoingData
        Call .WriteByte(ServerPacketID.UpdateOnline)
...
Private Sub HandleGlobalMessage(ByVal UserIndex As Integer)
   ...
   On Error GoTo HandleGlobalMessage_Error
   ...
   With UserList(UserIndex)
        ...
        If .flags.Silenciado Then Exit Sub

        Dim TActual As Long
        TActual = GetTickCount() And &H7FFFFFFF

        If Not (TActual - .UltimoGlobal) < (INTERVALO_GLOBAL * 1000) Then

          If GlobalActivado = 1 And .Pos.map <> Prision.map Then
              If .flags.Muerto = 1 Then
                  Call WriteConsoleMsg(UserIndex, "Tu estado no te permite enviar un mensaje global.", FontTypeNames.FONTTYPE_INFO)
                  Call .incomingData.CopyBuffer(Buffer): Exit Sub
              Else
                  If LenB(message) > 0 Then
                        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[" & .Name & "] " & message, FontTypeNames.FONTTYPE_TALK))
                        .UltimoGlobal = TActual
                  End If
              End If

          ElseIf GlobalActivado = 1 Then
          ...
          End If
      Else
        Call WriteConsoleMsg(UserIndex, "Debes esperar " & INTERVALO_GLOBAL & " segundos para enviar otro mensaje global.", FontTypeNames.FONTTYPE_INFO)
    End If
...
50  If Error <> 0 Then Err.Raise Error

51  Call LogError("Error " & Err.Number & " (" & Err.description & ") in procedure HandleGlobalMessage, line " & Erl & ".")
...
''
' Writes the "ShowFormViajes" message to the given user's outgoing data buffer.
'
' @param    UserIndex User to which the message is intended.
' @remarks  The data is not actually sent until the buffer is properly flushed.

Public Sub WriteShowFormViajes(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    '***************************************************
    'Author: Juan Mazotti (Lherkiev)
    'Last Modification: 01/10/19
    'Writes the "ShowFormViajes" message to the given user's outgoing data buffer
    '***************************************************
    On Error GoTo Errhandler

    Dim MAX_CIUDADES As Integer
    MAX_CIUDADES = UBound(Npclist(NpcIndex).Ciudades())

    If MAX_CIUDADES < 0 Then Exit Sub
    If MAX_CIUDADES > MAX_VIAJES Then Exit Sub

    Dim LoopC As Long, ID As Integer, Count As Byte
    ReDim ArrayCiudades(0 To MAX_CIUDADES) As Integer

    For LoopC = 0 To MAX_CIUDADES
        ID = Npclist(NpcIndex).Ciudades(LoopC)

        If ID > 0 Then
            If Viajes(ID).Pos.map <> Npclist(NpcIndex).Pos.map Then
                ArrayCiudades(Count) = ID
                Count = Count + 1
            End If
        End If
    Next LoopC
    ...
    With UserList(UserIndex).outgoingData
        ...
        Call .WriteByte(ServerPacketID.ShowFormViajes)
        Call .WriteByte(Count)

        For LoopC = 1 To Count
            ID = ArrayCiudades(LoopC - 1)

            If ID > 0 Then
                Call .WriteByte(ID)
                Call .WriteASCIIString(Viajes(ID).Nombre)
                Call .WriteByte(IIf(MapInfo(Viajes(ID).Pos.map).Pk, 0, 1))
                Call .WriteLong(Viajes(ID).Costo)
                Call .WriteInteger(Viajes(ID).Tiempo)
                Call .WriteByte(Viajes(ID).Nivel)
            End If
        Next LoopC
...
Private Sub HandleViajar(ByVal UserIndex As Integer)
    ...
    Dim Index As Byte, NpcIndex As Integer
    ...
    With UserList(UserIndex)
        ...
        With Npclist(NpcIndex)
            ...
            If .NPCtype <> eNPCType.Pasajes Then Exit Sub

            If Distancia(.Pos, .Pos) > 3 Then
                Call WriteConsoleMsg(UserIndex, "¡Tienes que acercarte más!", FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If

            ' ++ Checkeo para que no editen paquetes kb pt
            Dim MAX_CIUDADES_NPC As Integer
            MAX_CIUDADES_NPC = UBound(.Ciudades())

            If MAX_CIUDADES_NPC <> -1 Then
                Dim LoopC As Long, ID As Integer

                For LoopC = 0 To MAX_CIUDADES_NPC
                    ID = .Ciudades(LoopC)
                    If Index = ID Then
                        If Viajes(ID).Pos.map <> .Pos.map Then
                            Call m_Viajes.Viajar(UserIndex, Index)
                            Exit Sub
                        End If
                    End If
                Next LoopC
        ...
        Call LogError(.Name & " uso editor de paquetes en el sistema de viajes.")
...
Private Sub HandleLoginExistingAccount(ByVal UserIndex As Integer)
    ...
    'Remove packet ID
    Call Buffer.ReadByte '1

    Dim UserAccount As String
    Dim Password As String
    Dim Version As String

    UserAccount = Buffer.ReadASCIIString() '3
    Password = Buffer.ReadASCIIString() '5
    Version = CStr(Buffer.ReadByte()) & "." & CStr(Buffer.ReadByte()) & "." & CStr(Buffer.ReadByte()) '8

    'If we got here then packet is complete, copy data back to original queue
    Call UserList(UserIndex).incomingData.CopyBuffer(Buffer)
    
    If VersionOK(Version) Then
        Call ConnectAccount(UserIndex, UserAccount, Password)
    Else
        Call WriteErrorMsg(UserIndex, "Error de conexión, ejecute el AutoUpdate para solucionar el error, en todo caso vea las novedades en https://EvolutionAo.com.uy o en nuestro discord.")
    End If
...
Private Sub HandleLoginNewAccount(ByVal UserIndex As Integer)
    ...
    UserName = Buffer.ReadASCIIString() '3
    Password = Buffer.ReadASCIIString() '5
    Email = Buffer.ReadASCIIString() '7
    Pin = Buffer.ReadASCIIString() '9

    Version = CStr(Buffer.ReadByte()) & "." & CStr(Buffer.ReadByte()) & "." & CStr(Buffer.ReadByte()) '12

    If VersionOK(Version) Then
        Call CreateNewAccount(UserIndex, UserName, Password, Email, Pin)
    Else
    ...
    With UserList(UserIndex).outgoingData
        ...
        If NumberOfCharacters <> 0 Then
            For i = 1 To NumberOfCharacters
                ...
                Call .WriteByte(Characters(i).Estatus)
            Next i
        End If
...
Private Sub HandleRecoverAccount(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        AccountEmail = Buffer.ReadASCIIString
        AccountPin = Buffer.ReadASCIIString

        If LenB(AccountName) <> 0 And LenB(AccountEmail) <> 0 And LenB(AccountPin) <> 0 Then
            If CuentaExiste(AccountName) Then
                If UCase$(AccountEmail) = UCase$(GetVar(AccountPath & AccountName & ".acc", "INIT", "EMAIL")) And UCase$(AccountPin) = UCase$(GetVar(AccountPath & AccountName & ".acc", "INIT", "PIN")) Then
                    Call WriteErrorMsg(UserIndex, "Su personaje ha sido recuperado. Su nueva password es: " & GenerateRandomKey(AccountName))
                Else
...
Private Sub HandleChangePasswordAccount(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If LenB(newPass) <> 0 Then
            ...
            Call Leer.Initialize(AccountPath & .Account & ".acc")

            If UCase$(Leer.GetValue("INIT", "Password")) <> oldPass Then
                Call WriteConsoleMsg(UserIndex, "La contraseña actual proporcionada no es correcta. La contraseña no ha sido cambiada, inténtalo de nuevo.", FontTypeNames.FONTTYPE_INFO)
            Else
                Call Leer.ChangeValue("INIT", "Password", newPass)
                Call Leer.DumpFile(AccountPath & .Account & ".acc")

                Call WriteErrorMsg(UserIndex, "La contraseña fue cambiada con éxito.")
            End If
...
Private Sub HandleDeleteCharAccount(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If Index > 0 And LenB(AccountEmail) <> 0 And LenB(AccountPin) <> 0 Then

            Dim LeerAccount As clsIniManager
            Set LeerAccount = New clsIniManager
            Call LeerAccount.Initialize(AccountPath & .Account & ".acc")

            If StrComp(UCase$(AccountEmail), UCase$(LeerAccount.GetValue("INIT", "EMAIL"))) <> 0 Then
                Call WriteErrorMsg(UserIndex, "Error a comprobar datos, email no valido")
                Set LeerAccount = Nothing
                Exit Sub
            End If
            
            If StrComp(UCase$(AccountPin), UCase$(LeerAccount.GetValue("INIT", "PIN"))) <> 0 Then
                Call WriteErrorMsg(UserIndex, "Error a comprobar datos, pin no valido")
                Set LeerAccount = Nothing
                Exit Sub
            End If

            Dim CantPjs As Byte
            CantPjs = val(LeerAccount.GetValue("INIT", "CANT_PJS"))

            If Index > CantPjs Then Exit Sub

            AccountName = LeerAccount.GetValue("PJS", "PJ" & Index)

            If Not PersonajeExiste(AccountName) Then
                Call WriteErrorMsg(UserIndex, "El personaje no existe.")
                Exit Sub
            End If

            Dim tUser As Integer
            tUser = NameIndex(AccountName)

            If tUser > 0 Then
                Call WriteErrorMsg(UserIndex, "El personaje está logueado.")
                Call WriteConsoleMsg(tUser, "Un usuario intento borrar tu personaje.", FontTypeNames.FONTTYPE_FIGHT)
            Else
                If .GuildIndex < 1 Then
                    If KillCharINFO(AccountName, ErrorMsg) Then
                        Call LeerAccount.ChangeValue("INIT", "CANT_PJS", CantPjs - 1)
                        Call LeerAccount.ChangeValue("PJS", "PJ" & Index, "")

                        Dim i As Long, NameAux As String

                        For i = Index + 1 To 8
                            NameAux = LeerAccount.GetValue("PJS", "PJ" & i)
                            Call LeerAccount.ChangeValue("PJS", "PJ" & i - 1, NameAux)
                        Next i

                        Call LeerAccount.DumpFile(AccountPath & .Account & ".acc")
                        Set LeerAccount = Nothing

                        Call LoginAccountCharfile(UserIndex, .Account)
                        Call WriteErrorMsg(UserIndex, "Personaje Borrado Exitosamente.")
                        Call LogDesarrollo("El personaje: " & AccountName & " ha sido BORRADO. IP:" & .ip)
                    Else
                        Call LogDesarrollo("El personaje: " & AccountName & " ha intentado borrar su pj. IP:" & .ip)
                        Call WriteErrorMsg(UserIndex, ErrorMsg)
                    End If
                End If
            End If
...
Private Function KillCharINFO(ByVal UserName As String, ByVal ErrorMsg As String) As Boolean
    ...
    d = GetVar(App.Path & "\guilds\_guildsinfo.inf", "GUILD" & c, "founder")
    f = GetVar(App.Path & "\guilds\_guildsinfo.inf", "GUILD" & c, "GuildName")
    G = GetVar(App.Path & "\guilds\" & f & "-members.mem", "INIT", "NroMembers")
    j = GetVar(App.Path & "\guilds\" & f & "-members.mem", "Members", "Member" & G)

    If LenB(c) > 0 Then
        If d <> User Then
            Call guilds(c).ExpulsarMiembro(UserName)
        Else
            For h = 1 To G
                i = GetVar(App.Path & "\guilds\" & f & "-members.mem", "Members", "Member" & h)

                If i = User Then
                    Call WriteVar(App.Path & "\guilds\" & f & "-members.mem", "Members", "Member" & h, j)
                    Call WriteVar(App.Path & "\guilds\" & f & "-members.mem", "INIT", "NroMembers", G - 1)
                End If

                Call WriteVar(App.Path & "\guilds\_guildsinfo.inf", "GUILD" & c, "EleccionesAbiertas", "1")
                Call WriteVar(App.Path & "\guilds\_guildsinfo.inf", "GUILD" & c, "EleccionesFinalizan", DateAdd("d", 1, Now))
                Call WriteVar(App.Path & "\guilds\" & f & "-votaciones.vot", "INIT", "NumVotos", "0")
            Next h
        End If
    End If

    Call FileCopy(CharPath & UserName & ".chr", App.Path & "\CharfileDeleted\" & UserName & ".chr")

    If PersonajeExiste(UserName) Then
        KillCharINFO = True
        Call Kill(CharPath & UserName & ".chr")
    Else
        ErrorMsg = "Error al borrar la cuenta. Contacte con un Administrador para mejor asesoramiento."
        Exit Function
    End If
...
50  On Error GoTo HandleRequieredCaptions_Error
...
110     If .flags.Privilegios > PlayerType.User Then
120         tUser = NameIndex(UserName)

130         If tUser > 0 Then
140             Call WriteRequieredCAPTIONS(tUser)
150             UserList(tUser).ElPedidor = UserIndex
160             Call LogGM(.Name, "Pidió Captions a " & UserName)
170         Else
180             Call WriteConsoleMsg(UserIndex, "Usuario offline.", FontTypeNames.FONTTYPE_INFO)
190         End If
200     End If
...
270 Call LogError("Error " & Err.Number & " (" & Err.description & ") in procedure HandleRequieredCaptions, line " & Erl & ".")
...
Private Sub HandleSendCaptions(ByVal UserIndex As Integer)
   ...
   If UserList(UserIndex).incomingData.length < 3 Then
    ...
    On Error GoTo HandleSendCaptions_Error
   ...
   With UserList(UserIndex)
       ...
       Call Buffer.ReadByte
        ...
        Dim str As String
       str = Buffer.ReadASCIIString()

       If .ElPedidor > 0 Then
           Call WriteShowCaptions(.ElPedidor, str)
          .ElPedidor = 0
        Else
          Call CloseSocket(UserIndex)     'KB AO
        End If
...
HandleSendCaptions_Error:
  ...
  Call LogError("Error " & Err.Number & " (" & Err.description & ") in procedure HandleSendCaptions, line " & Erl & ".")

End Sub

Private Sub WriteShowCaptions(ByVal UserIndex As Integer, ByVal Caps As String)
...
Private Sub WriteRequieredCAPTIONS(ByVal UserIndex As Integer)

    On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.rCaptions)
    Exit Sub
...
Private Sub WriteSeeInProcess(ByVal UserIndex As Integer)
...
1   If UserList(UserIndex).incomingData.length < 3 Then
2       Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
        Exit Sub
    End If

    On Error GoTo HandleSendProcessList_Error

3   With UserList(UserIndex)

4       Dim Buffer As clsByteQueue: Set Buffer = New clsByteQueue
5       Call Buffer.CopyBuffer(.incomingData)
6       Call Buffer.ReadByte

        Dim data As String
7       data = Buffer.ReadASCIIString()

8       If .ElPedidor > 0 Then
9           Call WriteConsoleMsg(.ElPedidor, "Procesos de " & .Name & ": " & data, FontTypeNames.FONTTYPE_INFO)
10          .ElPedidor = 0
        Else
11          Call CloseSocket(UserIndex)     'KB AO
        End If

12      Call .incomingData.CopyBuffer(Buffer)

    End With

    Exit Sub

HandleSendProcessList_Error:
...
13  Error = Err.Number
    ...
    'Destroy auxiliar buffer
14  Set Buffer = Nothing

15  If Error <> 0 Then Err.Raise Error

16  Call LogError("Error " & Error & " (" & Err.description & ") in procedure HandleSendProcessList, line " & Erl & "." & " UserName: " & IIf(UserIndex > 0, UserList(UserIndex).Name, "Userindex < 1"))
...
50  On Error GoTo HandleLookProcess_Error
...
130         If tUser > 0 Then
140             Call WriteSeeInProcess(tUser)
150             UserList(tUser).ElPedidor = UserIndex
160             Call LogGM(.Name, "Pidió Procesos a " & UserName)
170         Else
180             Call WriteConsoleMsg(UserIndex, "Usuario offline.", FontTypeNames.FONTTYPE_INFO)
190         End If
200     End If
...
270 Call LogError("Error " & Err.Number & " (" & Err.description & ") in procedure HandleLookProcess, line " & Erl & ".")

End Sub

Private Sub HandleSendReto(ByVal User_Index As Integer)

    If UserList(User_Index).incomingData.length < 13 Then
        Err.Raise UserList(User_Index).incomingData.NotEnoughDataErrCode
        Exit Sub
    End If
    ...
    With UserList(User_Index)
        ...
        ByPoint = Buffer.ReadLong()
        snResu = Buffer.ReadBoolean()
        ...
        If UCase$(MyTeam) = UCase$(Enemy) Or _
           UCase$(MyTeam) = UCase$(tEnemy) Or _
           UCase$(Enemy) = UCase$(tEnemy) Or _
           UCase$(MyTeam) = UCase$(.Name) Or _
           UCase$(Enemy) = UCase$(.Name) Or _
           UCase$(tEnemy) = UCase$(.Name) Then

        Else

            If Not EsGm(User_Index) Then
                If EsGmChar(MyTeam) Then
                    Call .incomingData.CopyBuffer(Buffer)
                    Exit Sub
                End If

                If EsGmChar(Enemy) Then
                    Call .incomingData.CopyBuffer(Buffer)
                    Exit Sub
                End If

                If EsGmChar(tEnemy) Then
                    Call .incomingData.CopyBuffer(Buffer)
                    Exit Sub
                End If
            End If

            Call m_Retos2vs2.set_reto_struct(User_Index, MyTeam, Enemy, tEnemy, ByDrop, ByGold, ByPoint, snResu)

            If (m_Retos2vs2.Can_Send_Reto(User_Index, sError) = True) Then
                Call m_Retos2vs2.Send_Reto(User_Index)
            Else
...
Private Sub HandleAcceptReto(ByVal User_Index As Integer)
    ...
    If UserList(User_Index).incomingData.length < 3 Then
        Err.Raise UserList(User_Index).incomingData.NotEnoughDataErrCode
        Exit Sub
    ...
    With UserList(User_Index)
        ...
        ' @@ Cuicui
        If (UCase$(.mReto.Request_Name) = cName) Then Es1vs1 = True
        If (UCase$(.sReto.Nick_Sender) = cName) Then Es2vs2 = True

        If Es1vs1 And Es2vs2 Then
            If .mReto.AcceptLimitCount > .sReto.AcceptLimit Then
                Es2vs2 = False
            Else
                Es1vs1 = False
            End If
        ...
        If Es2vs2 Then Call m_Retos2vs2.Accept_Reto(User_Index, cName)
        If Es1vs1 Then Call m_Retos1vs1.Accept_Reto(User_Index, cName)
...
Private Sub HandleOtherSendReto(ByVal UserIndex As Integer)

10  On Error GoTo HandleOtherSendReto_Error

20  If UserList(UserIndex).incomingData.length < 9 Then
...
50  End If
    ...
    Dim eName As String
    Dim g_gold As Long
    Dim g_Points As Long
    Dim g_drop As Byte
    Dim mError As String

60  With UserList(UserIndex)

70      Dim Buffer As clsByteQueue
        Set Buffer = New clsByteQueue
80      Call Buffer.CopyBuffer(.incomingData)
...
100     eName = Buffer.ReadASCIIString()
110     g_gold = Buffer.ReadLong()
120     g_drop = Buffer.ReadByte() = 1
180     g_Points = Buffer.ReadLong()

290     If (m_Retos1vs1.Can_Send_Reto(UserIndex, eName, g_gold, mError, g_Points) = True) Then
300         Call m_Retos1vs1.Send_Reto(UserIndex, NameIndex(eName), g_gold, g_drop, g_Points)
310     Else
            If LenB(mError) > 0 Then
320             Call WriteConsoleMsg(UserIndex, mError, FontTypeNames.FONTTYPE_INFO)
            End If
330     End If
...
HandleOtherSendReto_Error:

370 Set Buffer = Nothing

380 Call LogError("Error " & Err.Number & " (" & Err.description & ") in procedure HandleOtherSendReto of Módulo Protocol " & Erl & ".")

End Sub

Private Sub WriteUpdateSegHP(ByVal UserIndex As Integer)

    On Error GoTo Errhandler

    With UserList(UserList(UserIndex).ElPedidorSeguimiento)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateSegHP)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MinHp)
    End With
    Exit Sub
...
Private Sub WriteUpdateSegMANA(ByVal UserIndex As Integer)

    On Error GoTo Errhandler
    With UserList(UserList(UserIndex).ElPedidorSeguimiento)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateSegMANA)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MinMAN)
    End With
    Exit Sub
...
Private Sub WriteUpdateSegInitial(ByVal UserIndex As Integer)

    On Error GoTo Errhandler

    With UserList(UserList(UserIndex).ElPedidorSeguimiento)
        Call .outgoingData.WriteByte(ServerPacketID.UpdateSegInitial)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MinHp)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MaxHp)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MinMAN)
        Call .outgoingData.WriteInteger(UserList(UserIndex).Stats.MaxMAN)
    End With
...
Private Sub HandleSeguimiento(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        If .flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios Or PlayerType.Consejero) Then
            ...
            If UserName = "1" Then
                If .Siguiendo > 0 Then
                    UserList(.Siguiendo).ElPedidorSeguimiento = 0
                    .Siguiendo = 0
                    Call WriteConsoleMsg(UserIndex, "Has dejado de seguir al usuario.", FontTypeNames.FONTTYPE_SERVER)
                End If
            ...
            If TargetIndex > 0 Then
                ...
                If UserList(TargetIndex).ElPedidorSeguimiento > 0 Then
                    Call WriteConsoleMsg(UserList(TargetIndex).ElPedidorSeguimiento, "El GM " & .Name & " ha comenzado a seguir al usuario que estabas siguiendo.", FontTypeNames.FONTTYPE_SERVER)
                End If

                ' @@ Si estaba siguiendo a un usuario y se pone a seguir a otro
                If .Siguiendo > 0 Then
                    Call WriteConsoleMsg(UserIndex, "Dejas de seguir a " & UserList(.Siguiendo).Name & "." & vbNewLine & _
                                                    "Comienzas a seguir a " & UserList(TargetIndex).Name & ".", FontTypeNames.FONTTYPE_SERVER)

                    UserList(.Siguiendo).ElPedidorSeguimiento = 0
                End If

                UserList(TargetIndex).ElPedidorSeguimiento = UserIndex
                UserList(UserIndex).Siguiendo = TargetIndex

                Call WriteUpdateSegInitial(TargetIndex)
            End If

            Call .incomingData.CopyBuffer(Buffer)

        Else
            Call .incomingData.RemoveData(3)
            Exit Sub
        End If
...
Private Sub HandleSolicitarRanking(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        Call WriteSendRanking(UserIndex, .incomingData.ReadByte)
    End With

End Sub

Public Sub WriteSendRanking(ByVal UserIndex As Integer, ByVal Rank As eRanking)
    ...
    Cadena = Cadena & Ranking(Rank).Nombre(1)
    Cadena2 = Cadena2 & Ranking(Rank).Value(1)
    ...
    Call UserList(UserIndex).outgoingData.WriteByte(ServerPacketID.SendDatosRanking)
...
Public Function PrepareMessageCuentaRegresiva(ByVal CountTime As Integer) As String

    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.CuentaRegresiva)
        Call .WriteInteger(CountTime)
        PrepareMessageCuentaRegresiva = .ReadASCIIStringFixed(.length)
    End With
...
Private Sub HandleCuentaRegresiva(ByVal UserIndex As Integer)
    ...
    If MapaValido(Mapa) Then
        Call SendData(SendTarget.toMap, Mapa, PrepareMessageCuentaRegresiva(Cuenta))
    Else
        Call SendData(SendTarget.ToAll, 0, PrepareMessageCuentaRegresiva(Cuenta))
    End If
    
End Sub

Public Sub WriteCaptchaCode(ByVal UserIndex As Integer)

    On Error GoTo Errhandler

    With UserList(UserIndex)

        Call .outgoingData.WriteByte(ServerPacketID.CaptchaCode)
        
        ' 4 digitos.... letra-numero-letra-numero
        'If .CaptchaCode(0) = 0 Then
            .CaptchaCode(0) = RandomNumber(97, 122)
            .CaptchaCode(1) = RandomNumber(48, 57)
            .CaptchaCode(2) = RandomNumber(97, 122)
            .CaptchaCode(3) = RandomNumber(48, 57)
        'End If
        
        Call .outgoingData.WriteByte(.CaptchaCode(0))
        Call .outgoingData.WriteByte(.CaptchaCode(1))
        Call .outgoingData.WriteByte(.CaptchaCode(2))
        Call .outgoingData.WriteByte(.CaptchaCode(3))
        
        .LastTickCaptcha = GetTickCount() And &H7FFFFFFF
