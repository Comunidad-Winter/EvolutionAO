...
Public Const MAX_OFFER_SLOTS As Integer = 20 '30    '20
...
Public Sub IniciarComercioConUsuario(ByVal Origen As Integer, ByVal Destino As Integer)
    ...
    If UserList(Origen).ComUsu.DestUsu = Destino And _
       ...
       If UserList(Origen).Pos.map <> 192 Then 'MAPA PARA COMERCIAR MAPACOMERCIO = 192
            Call WriteConsoleMsg(Origen, "No puedes comerciar si no estas en el mapa de comercio seguro.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
