    ...
    For i = 1 To UserList(UserIndex).CurrentInventorySlots
        ...
        If ObjIndex > 0 Then
            If (ObjData(ObjIndex).OBJType <> eOBJType.otLlaves And _
                ...
                ObjData(ObjIndex).Intransferible < 1) Then
...
Sub QuitarNewbieObj(ByVal UserIndex As Integer)
    ...
    'Last Modification: -
    '
    'Dungeon Newbie dejamos el hogar jaja
    '***************************************************
    ...
    With UserList(UserIndex)
        If num > 0 Then
            ...
            If ObjData(DropObj.ObjIndex).Intransferible = 1 Then
               If .flags.Privilegios And (PlayerType.User Or PlayerType.SemiDios Or PlayerType.Consejero) Then
                    Call WriteConsoleMsg(UserIndex, "¡¡¡No puedes tirar este tipo de objeto!!!", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
            End If
...
Sub UseInvItem(ByVal UserIndex As Integer, ByVal Slot As Byte, ByVal ByClick As Byte)
    ...
    With UserList(UserIndex)
        ...
        'Si está en carcel no puede usar nada.
        ...
        Select Case Obj.OBJType
            ...
            Case eOBJType.otBarcos
                ...
                If .Stats.ELV < 25 Then
                    If .clase <> eClass.Worker And .clase <> eClass.Pirat Then
                        Call WriteConsoleMsg(UserIndex, "Para poder navegar necesitas ser nivel 25 o superior y tener 70 skills en navegación.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    Else
                        If .Stats.ELV < 25 Then
                            Call WriteConsoleMsg(UserIndex, "Para poder navegar necesitas ser nivel 25 o superior y tener 70 skills en navegación.", FontTypeNames.FONTTYPE_INFO)
            ...
            Case eOBJType.otObjetoViajero
                ...
                If .Counters.ViajeOBJ <> 0 Then
                    Call WriteConsoleMsg(UserIndex, "Ya tienes un viaje, llegaras en " & .Counters.ViajeOBJ & " segundos.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If

                If MapInfo(.Pos.map).Pk Or EstaEnCiudad(.Pos.map) Then
                    If .flags.Muerto < 1 Then
                        Call WriteConsoleMsg(UserIndex, "Debes estar muerto o en una zona segura para usar éste objeto viajero aquí.", FontTypeNames.FONTTYPE_INFO)
                        Exit Sub
                    End If
                End If

                .Counters.ViajeOBJ = IIf(MapInfo(.Pos.map).Pk, 5, 2)    'Tiempo hogar para teletransportarte
                .PosViaje = ObjData(ObjIndex).PasajePos

                Call WriteConsoleMsg(UserIndex, "En " & .Counters.ViajeOBJ & " segundos seras teletrasportado.", FontTypeNames.FONTTYPE_INFO)
...
Private Function EstaEnCiudad(ByVal map As Integer) As Byte

    Dim LoopC As Long

    For LoopC = 1 To NUMCIUDADES
        If Ciudades(LoopC).map = map Then
            EstaEnCiudad = 1
            Exit Function
        End If
    Next LoopC
...
Sub TirarTodo(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        'If Not Cantidad > 100000 Then    'Si el oro no es 100k, se cae..
...
Public Function ItemSeCae(ByVal Index As Integer) As Boolean
    ...
    With ObjData(Index)
        ItemSeCae = (.Real <> 1 Or .NoSeCae = 0) And _
                    ...
                    .NoSeCae = 0 And .Intransferible = 0
    End With
